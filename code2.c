...
TASK(TaskClock)
{
	...
	/* Link the variables to behavioral model (hand-coded) */

	/*
	 * Step the SM
	 */

	rt_OneStep(button[4], button[5], button[0], button[1], button[2], button[3], tick, alarmSrv);
...
int main(void)
{
	...
	/*
	 * ------------------------------ State Machine initialization (hand-coded) ------------------------------
	 */

	alarmDrv = false;

	rt_Init(hmst, &mode, &sel, &alarmDrv);
